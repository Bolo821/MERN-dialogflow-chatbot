...
function PlanDialog(props) {
	...
	const user_orders = useSelector(({plansApp}) => plansApp.contacts.entities);
	const guest_orders = useSelector(({user}) => user.guest.orderHistory);

	useEffect(() => {
		if(user.email) {
			setOrders(user_orders);
		} else {
			setOrders(guest_orders);
		}
	}, [orders, user_orders, guest_orders])
		...
		>
			...
			<Card className={clsx("w-full rounded-8 shadow ", classes.root)}>
				<div className="p-16 pb-0 flex flex-row flex-wrap items-center justify-center">
					...
					{contactDialog.orderNum >= 0 && orders.length ?
